...
<c:url value="/canva?acao=AdicionarCusto" var="linkAddParc" />
<html lang="pt-br">
  ...
  <head>
    ...
    <title>Estrutura de custos - Canva Generator</title>
  ...
  <body class="centro" style="width: 95%; background: linear-gradient(to right, #631A52, #38006B)">
    ...
    <div class="row">
      ...
      <div class="col-md-8">
        <h1 class="text-white">Estrutura de custos</h1>
        <div class="mb-3">
          <form action="${linkAddParc}" method="POST">
            <textarea class="form-control" id="exampleFormControlTextarea1" rows="7"
                      <c:if test="${lista.size() < 1}">placeholder="Digite aqui seus parceiros chave"</c:if> style="resize: none;"
                      name="descricao" <c:if test="${lista.size() == 1}">placeholder="ESTE ITEM JÁ FOI ADICIONADO" disabled</c:if>></textarea>
